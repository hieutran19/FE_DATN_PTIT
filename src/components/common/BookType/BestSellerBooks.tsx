...
const BestSellerBooks = () => {
  ...
  useEffect(() => {
    const handleFetchBook = async () => {
      const response = await fetch(API_ENDPOINT + `/books?sortBy=amount_borrowed:desc`)
  ...
  return (
    <div className="my-4 rounded-md bg-white p-4 shadow-lg">
      <div className="my-4 flex justify-between border-b-1 pb-2 text-slate-500">
        <p className="font-semibold">Truyện được muợn nhiều nhất</p>
        <Link href={`/all-book/bestSeller`} className="flex items-center gap-1 hover:text-blue-500">
